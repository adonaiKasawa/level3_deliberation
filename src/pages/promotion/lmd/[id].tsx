            ...
            <div className="row">
              {loading ?
                <div className="col-4 offset-4">
                  <img src={`${config.url}loading.gif`} alt="loading" className="img-responsive" width="100" />
                </div>
                : null}
              {error.existe ?
...
const PromotionLmdId = ({ unitPromTitRels, etudiants, deliberationPromotion, deliberationEtudiant, readDeliberationEtudiants }: Props) => {
  ...
  const [onUnite, setOnUnite] = useState(cpt[0]?.unite_ensiegnement.nom);
  const ExerciceEncours = useAppSelector(selectExerciceEncours);

  const handleChange = (i: number) => {
    setReleves(cpt[i]?.releves);
    setOnUnite(cpt[i]?.unite_ensiegnement.nom)
  }
  ...
  const handleDeleberation = async () => {
    ...
    etudiants.map((item: any, i: number) => {
      ...
      etudiantDeliberer.push({
        ...
        poucentageCredit: POURCENTAGECREDIT,
        descision: DECISION,
  ...
  const confirmAllRecoursForDeliberation = async (deliberationId: number, session: number) => {
    if (session == 2) {
      ...
      vous mettez fin à la deliberation et vous procedez au passage de promotion 
      automatique.
    `)) {
        const confirmAllRecoursForDeliberation = await ConfirmAllRecoursForDeliberationId(deliberationId, ExerciceEncours.id_exercice);
        const data = confirmAllRecoursForDeliberation?.data;
        console.log(data);
        if (data !== undefined && confirmAllRecoursForDeliberation.statusText == 'OK') {
          const confirmCursus = await changeThePromotionAndHandleCursus(deliberationId)
          console.log(confirmCursus);
    ...
    if (deliberation > 0) {
      if (deliberation == 1) {
        ...
        if (status) {
          ...
          return <button className="btn btn-success" onClick={handleDeleberation}>Deliber pour la 2<sup>eme</sup> semestre</button>
        }
        ...
        if (status) {
          ...
          className="btn btn-warning"
          onClick={() => confirmAllRecoursForDeliberation(id, session)}>
          Confirmer tout les recours de la deuxieme semestre!</button>
        }
      }
    } else {
      return <button className="btn btn-primary" onClick={handleDeleberation}>Déliberer pour le 1 <sup>ér</sup> semestre!</button>
    }
  }
  ...
  return (
    <div>
      <div className="col-10 offset-1">
        ...
        <h6>Promotion {promotion?.nom_promotion} {promotion?.systeme} {promotion?.vacation}</h6>
        {checkIfPromotionIsDeliberated()}
        <hr />
      </div>
      <div className="row mb-2">
        <div className="col-md-6">
          {semestre == 2 ? <button className="btn btn-primary col-md-6"
            onClick={() => handleChangeSemestre(1)}>1<sup>ere</sup> semestre</button>
            :
            <button className="btn btn-success col-md-6"
              onClick={() => handleChangeSemestre(2)}>2<sup>eme</sup> semestre</button>}
                ...
                {cpt?.map((item: any, i: number) => {
                  if (item.semestre === semestre) {
                    return <li className="nav-item"
                      ...
                      <a className={`nav-link  ${onUnite == item.unite_ensiegnement.nom ? 'active' : null}`}>
        ...
        <div className="col-md-8 col-ms-8 col-12">
          <div className="card card-primary">
                        ...
                        return (
                          <tr key={i}>
                            ...
                            <td>
                              ...
                              <Link href={`${config.url}${getLinkReleve()}/${item.id_etudiant}?semestre=${semestre}&exercice=${ExerciceEncours.id_exercice}`}>
            ...
            <div className="card-footer">
              {possibiliteToUpdate ? updateAllCote ?
                loading ? <p>loading... please wait</p> :
                  <>
                    <button className="btn btn-success" onClick={() => AllUpdateCoteByUniteEnseignment()}>Confirmer la modification</button>
                    <button className="btn btn-danger" onClick={() => setUpdateAllCote(!updateAllCote)}>Annuler la modification</button>
                  </>
