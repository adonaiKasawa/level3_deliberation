...
import { ConfirmAllRecoursForDeliberationId, CotationRatrapageUniteData, CotationUniteData, creatDeliberationEtudiant, CreatDeliberationEtudiantData, creatDeliberationPromotion, createCotation, createRatrapageCotation, readDeliberationByEtudiants, readDeliberationByPromtion, readDeliberationPromotionById, readEtudiantByPromotionId, readUnitPromTitulaireById, readUnitPromTitulaireByPromotionId, RelevesData, updateReleves } from "../../services";
...
const updateModale = ({
  ...
  const handleSubmit = async () => {
    setSuccess(false);
    try {
      const CreateCotation = await createCotation([{
        tj: releves.tj,
        ex: releves.ex,
        etudiantId,
        motif
...
const CoursId = ({
  ...
  const AllUpdateCoteByUniteEnseignment = async () => {
    ...
    const allUpdate: CotationRatrapageUniteData[] = []
    const preAllUpdate = [];
    let error = false;
    ...
    for (let i = 0; i < preAllUpdate.length; i++) {
      ...
      const find = allUpdate.find(item => item.releveId === etudiant.releveId);
      if (find) {
        if (find?.ex) {
          find.tj = etudiant.tj
        } else if (find?.tj) {
    ...
    try {
      const CreateCotation = await createRatrapageCotation(allUpdate);
      if (CreateCotation?.data !== undefined && CreateCotation?.statusText === "Created") {
  ...
  const UpdateAllState = async () => {
    const getUpt = await readUnitPromTitulaireById(unitPromTitRels.id_unit_prom_titulaire, ExerciceEncours.id_exercice);
  ...
  return (
    <div>
      <div className="col-10 offset-1">
        <h6> Cours : {onUnite}  </h6>
        <h6> Promotion : {promotion?.nom_promotion} {promotion?.departement.nom_departement}</h6>
        <h6> Vacation : {promotion?.vacation}</h6>
        <h6>annee academique: {unitPromTitRels?.exercice?.annee_exercice}</h6>
        <hr />
      </div>
      <div className="row">
            ...
            {unitPromTitRels !== null ? <>
              <div className="card-body table-responsive p-0" style={{ height: 300 }}>
                <table className="table table-head-fixed p-0">
                  ...
                  <tbody>
                    {releves && releves?.map((item: any, i: number) => {
                      let tj = item.tj;
                      let ex = item.ex;
                      let etudiantid = item.id_etudiant
                      let releveId = 0
                      let releve_Modifications = item.releveModifications;
                      let isDeliberated = false;
                      let descision = '';
                      let pts = item.tj + item.ex
                      if (pts < 10 && item.validation !== true) {
                        e++
                        return (
                          <tr key={i}>
                            <td>{e}</td>
                            <td>{item.etudiant.nom_etudiant} {item.etudiant.postnom_etudiant} {item.etudiant.prenom_etudiant}</td>
                            <td>{updateAllCote ?
                              <div className="form-group">
                                <label htmlFor="nom">Examen</label>
                                <input
...
export const getServerSideProps = async (route: any) => {
  const unitPromTitRel = await readUnitPromTitulaireById(route.query.id, route.query.exercice);
  let dataUpt = null
