...
const Cours = ({sections}: Props) => {
  ...
  const getCours =  async () => {
    ...
    const getcours = await readUniteByTitulaire(exercice.id_exercice);
        ...
        <div className="row">
          {cours?.map((item: any, i: number) =>
            <div className="col-md-4 col-sm-6 col-12" key={i}>
              <Link href={`${config.url}cours/${item.id_unit_prom_titulaire}?exercice=${item.exercice.id_exercice}`}>
